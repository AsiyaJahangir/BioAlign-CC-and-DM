...
def get_degree_centrality(node, network):
    degree = 0
    for edge in network:
        if node in edge:
            degree += 1
...
def alignment_degree_centrality(remaining1, remaining2, network1, network2, aligned):
    degrees1 = [(node, get_degree_centrality(node, network1)) for node in remaining1]
    degrees2 = [(node, get_degree_centrality(node, network2)) for node in remaining2]
    
    # Sort by degree
    degrees1.sort(key=lambda x: x[1], reverse=True)
    degrees2.sort(key=lambda x: x[1], reverse=True)
    
    new_alignments = []
    for node1, degree1 in degrees1:
        best_match = None
        min_diff = float('inf')
        for node2, degree2 in degrees2:
            if node2 not in [a[1] for a in new_alignments]:
                diff = abs(degree1 - degree2)
                if diff < min_diff:
                    min_diff = diff
                    best_match = node2
        if best_match:
            new_alignments.append((node1, best_match))
    
    aligned.extend([f"{a[0]}\t{a[1]}" for a in new_alignments])
    return aligned
...
def main(specie1, specie2, choice="bt"):
    ...
    if choice == 'T' or choice == 't':
        print("Method: Stage-1 + Topology -- Degree Centrality Matching")
    ...
    if len(aligned_stage1) < max_length*0.99 and (choice == "t" or choice == "T"):
        print("\nStage-2 [Topology -- Degree Centrality Matching]")
        ...
        with open(f"./networks/{specie1}.interaction") as f:
            network1 = [line.strip().split() for line in f]
        with open(f"./networks/{specie2}.interaction") as f:
            network2 = [line.strip().split() for line in f]
        
        aligned_stage2 = alignment_degree_centrality(remaining1, remaining2, network1, network2, aligned_stage1)
        print("Nodes Aligned After Stage-2", len(aligned_stage2))
    
    elif len(aligned_stage1) < max_length*0.99 and (choice == "b" or choice == "B"):
        print("\nStage-2-i [Remote Homology]")
    ...
    elif len(aligned_stage1) < max_length*0.99:
        print("\nStage-2-i [Remote Homology]")
        net1, net2 = get_remaining_nodes(specie1, specie2, aligned_stage1)
        net1_proteins = get_values(specie1, net1)
        net2_proteins = get_values(specie2, net2)
        ...
        print("\nStage-2-iii [Topology -- Degree Centrality Matching]")
        ...
        aligned_stage2 = alignment_degree_centrality(remaining1, remaining2, network1, network2, aligned_stage2)
        print("Nodes Aligned After Stage-2", len(aligned_stage2))
    ...
    with open(f"./new_alignments/{specie1}-{specie2}-{choice}.alignment", 'w') as f:
        for alignment in aligned_stage2:
            f.write(f"{alignment}\n")
        f.close()   
